...
func (server *Server) Register(rcvr interface{}) error {
	...
	if _, dup := server.serviceMap.LoadOrStore(s.name, s); dup {
		return fmt.Errorf("rpc: service already defined: %v", s.name)
	}
	return nil
}
...
func (server *Server) findService(serviceMethod string) (svc *service, mtype *methodType, err error) {
	...
	if dot < 0 {
		err = fmt.Errorf("rpc server: service/method request ill-formed: %v", serviceMethod)
		return
	}
	serviceName, methodName := serviceMethod[:dot], serviceMethod[dot+1:]
	svci, ok := server.serviceMap.Load(serviceName)
	if !ok {
		err = fmt.Errorf("rpc server: can't find service %v", serviceName)
		return
	}
	svc = svci.(*service)
	mtype = svc.method[methodName]
	if mtype == nil {
		err = fmt.Errorf("rpc server: can't find method %v", methodName)
	}
	return
}
